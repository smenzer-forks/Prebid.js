...
function user(bidRequest, bidderRequest) {
  ...
  if (bidRequest) {
    if (bidRequest.userId) {
      ...
      // id5
      if (bidRequest.userId.id5id && bidRequest.userId.id5id.universal_uid) {
        addExternalUserId(ext.eids, bidRequest.userId.id5id.universal_uid, 'id5-sync.com', utils.deepAccess(bidRequest, 'userId.id5id.ext'));
      }
